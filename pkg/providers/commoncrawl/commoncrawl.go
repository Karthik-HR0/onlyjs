...
// Fetch fetches all .js urls for a given domain and sends them to a channel.
...
// formatURL adds the .js filter to the query to fetch only JavaScript files.
func (c *Client) formatURL(domain string, page uint) string {
	...
	// Append ".js" to ensure only JavaScript files are returned
	return fmt.Sprintf("%s?url=%s/*&output=json&fl=url&page=%d&urlFilter=*.js", c.apiURL, domain, page) + filterParams
}
...
// Helper function to check if a URL ends with .js
